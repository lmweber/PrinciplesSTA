...
- *Neutrality*: workflows should largely rely on standard methods that have been demonstrated to perform well in independent benchmarks, to avoid perceived bias towards methods developed by specific groups
...
In particular, we welcome contributions of [Workflows]. These are chapters in OSTA that each describe a complete workflow, consisting of several individual analysis steps. For example, these may be focused on a specific type of technological platform, or designed to showcase a certain type of analysis.
...
If you decide to contribute a new workflow, we ask that you first open a [GitHub issue](https://github.com/lmweber/OSTA-book/issues) to propose the idea along with an outline of your proposed workflow. The OSTA team can then give you initial feedback and you can proceed to make a pull request on GitHub. For new workflows, we recommend re-using code from the individual analysis chapters within OSTA (e.g. for standard analysis steps), which will make the examples easier for readers to follow.
