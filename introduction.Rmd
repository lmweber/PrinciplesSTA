...
- *Neutrality*: workflows should rely on standard methods that have been demonstrated to perform well in independent benchmarks, to avoid perceived bias towards methods developed by specific groups
- *Bioconductor-based*: workflows should be based largely on Bioconductor packages, where possible
- *Data structures*: examples should make use of Bioconductor data structures, i.e. [SpatialExperiment](https://bioconductor.org/packages/SpatialExperiment)
...
In particular, we welcome contributions to [Workflows]. These are chapters in OSTA that describe a complete workflow, consisting of several individual analysis steps. For example, these may be focused on a specific type of technological platform, or designed to showcase a certain type of analysis.

If you want to demonstrate the use of a new method (e.g. spatially aware clustering in a package you have developed), we recommend adding it to the relevant workflow(s) to compare against other methods performing a similar task.

If you decide to add to an existing workflow, the new function should run relatively fast (seconds to minutes) on the given dataset, due to limitations on the time it takes to build the book.

If you decide to contribute a new workflow, we ask that you open up a [GitHub issue](https://github.com/lmweber/OSTA-book/issues) to propose this idea with an outline of the workflow you propose to build. The OSTA team can give you initial feedback and then you can proceed to make a pull request. For new workflows, we also recommend re-using code from the individual analysis chapters within OSTA (e.g. for standard analysis steps) to make the examples easier for readers to follow.
