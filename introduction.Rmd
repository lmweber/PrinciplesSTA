...
In our view, one of the main advantages of the Bioconductor framework is the modularity and open development philosophy. R packages implementing analysis methods are contributed by research groups throughout the world, while the Bioconductor Core Team coordinates the overall project and maintains infrastructure such as data object structures and package testing infrastructure. A key feature is that contributed packages are required to use consistent data structures, such as the [SingleCellExperiment](http://bioconductor.org/packages/release/bioc/vignettes/SingleCellExperiment/inst/doc/intro.html) object class, which allows packages from different research groups to interact nicely (e.g. output from one package can easily be provided as input to another package). In addition, Bioconductor packages are required to have comprehensive documentation, including "vignettes" (long-form examples including R code).

This modular and open-development approach allows end users to readily combine and substitute analysis methods contributed by different research groups, instead of being "locked in" to using methods or packages maintained by a single group. For example, if a new and improved method is developed for one of the steps described in this book, then this can easily be substituted into the appropriate step in the pipeline, using the `SingleCellExperiment` data structure as the conduit. In addition, advanced users are welcome to contribute their own packages to Bioconductor.
...
We use the `SingleCellExperiment` Bioconductor object class throughout the analysis pipeline and workflows described in the main chapters of this book.

A summary of the `SingleCellExperiment` object structure is shown in the following figure. A `SingleCellExperiment` object consists of one or more `assays` (tables of raw and/or transformed data), `rowData` and `colData` (metadata describing features and cells), `reducedDims` (reduced dimension features), and optional additional `metadata`. The different parts of the object are connected, which enables convenient operations such as subsetting the entire object by rows (features) or columns (cells).
