...
Several new statistical methods adapted to identify SVGs in ST data have also recently been developed. However, in our view, these are not yet ready to integrate into the standardized pipelines we aim to demonstrate in OSTA. For example, some methods do not scale to large datasets with thousands of spatial coordinates, and others are only available as GitHub packages that do not meet Bioconductor standards of robustness / testing / documentation. In the future, we will expand this section to include additional examples as these methods become further developed.
