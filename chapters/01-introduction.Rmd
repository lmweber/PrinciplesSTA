...
This book is intended for readers who have some experience with R, but does not assume familiarity with Bioconductor. The examples and workflows include R code required to download data, set up data objects, perform analyses, and visualize results.
...
We welcome suggestions for updates to the analysis and workflow chapters in OSTA. Suggestions may be provided as [GitHub issues](https://github.com/lmweber/OSTA-book/issues). All methods included in OSTA should be available as R packages from [Bioconductor](http://bioconductor.org/) or [CRAN](https://cran.r-project.org/), which ensures compatibility with the existing workflows and provides users with guarantees regarding ease of installation, long-term availability, stability, and maintenance through the regular build-testing, checks, and maintenance checks provided by Bioconductor and CRAN. Additional methods available as packages from GitHub or other repositories may also be described in the analysis chapters, but these methods will not be integrated into the code examples. Methods available as `pip` installable Python packages will also be integrated into the examples in future updates of OSTA using [reticulate](https://rstudio.github.io/reticulate/).
